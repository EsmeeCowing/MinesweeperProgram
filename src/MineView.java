...
class MineView
{
  ...
  private class GamePanel extends JPanel
  {
    ...
    {   
      ...
      if (cell.isFlagged()) 
      {
        // fill a cell with the color pink as a representation of a flag
    ...
    //creates end bug bc it checks in every mouse click if game is won - i think i fixed it DELETE THIS COMMENT
    class MineMouseListener implements MouseListener 
    {
      public void mousePressed(MouseEvent event) 
      {
        if (!mineModel.isGameStarted() || mineModel.isGameOver() || mineModel.isGameWon()) return;
        int x = event.getX();
        int y = event.getY();
        ...
        if (mineModel.isPlayerDead()) {
          JOptionPane.showMessageDialog(mainFrame, "Game Over! You stepped on a mine.");
        }
        else if (mineModel.isGameWon()) {

          JOptionPane.showMessageDialog(mainFrame, "Good job!  You won the game!");
        }
      }
